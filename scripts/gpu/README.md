...
Settings come from `.env` (gitignored; `FV_ENV_FILE` points elsewhere), and shell variables take precedence: `VAST_API_KEY` (required; overrides the `vastai` CLI's saved key), `MAX_DPH`, `MAX_MINUTES`, `VAST_IMAGE`, `VAST_IMAGE_REPO`, `VAST_IMAGE_FALLBACK`, `VAST_SSH_KEY`. The key is exported only to local `vastai` calls. `.env` is excluded from the repo sync, so it never reaches the rented box.
...
- **GHCR runtime image:** `.github/workflows/gpucheck-runtime-image.yml` publishes on path-filtered pushes to any branch (and on `workflow_dispatch`). **Packages are not shared:** `main` owns `ghcr.io/<owner>/fastvideo-rs-runtime`; other branches publish `ghcr.io/<owner>/fastvideo-rs-runtime-<sanitized-ref>` (e.g. `cursor/flux2-warm-fused-sdpa-d37e` → `fastvideo-rs-runtime-cursor-flux2-warm-fused-sdpa-d37e`). Each run pushes `:build-<id>`, `:sha-<short>`, and `:latest` on **that package only**. Non-main refs never push to `fastvideo-rs-runtime`. `validate.sh run` prefers this ref's `:build-<id>`, then this package's `:latest` (upload local binary), and never main's `:latest` from a feature branch. If the branch package is missing, it uses `nvidia/cuda:12.4.1-cudnn-runtime-ubuntu22.04` (`VAST_IMAGE_FALLBACK`) and uploads the binary. Override the package with `VAST_IMAGE_REPO`. First publish of a new branch package may be private until it is made public in GHCR.
