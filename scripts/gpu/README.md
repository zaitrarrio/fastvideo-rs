...
- **GHCR runtime image:** `.github/workflows/gpucheck-runtime-image.yml` publishes `ghcr.io/<owner>/fastvideo-rs-runtime` on path-filtered pushes to any branch (and on `workflow_dispatch`). Every run pushes `:build-<id>` and `:sha-<short>`; feature branches also get `:branch-<sanitized-ref>`. `:latest` is moved only on `main`. `validate.sh run` prefers `:build-<id>` so the rented box matches local sources; if that tag is not up yet it falls back to `:latest` and uploads the local binary.
