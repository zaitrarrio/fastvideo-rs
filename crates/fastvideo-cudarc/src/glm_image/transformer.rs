//! Minimal GLM-Image DiT graph (tiny zeros + load hook).
...
impl GlmImageTransformer {
    ...
    pub fn load(cfg: GlmImageTransformerConfig, _map: &WeightMap) -> Result<Self> {
        let _ = _map;
        Self::zeros(cfg)
    }
    ...
    pub fn forward(
        ...
        if c != self.cfg.in_channels {
            return Err(msg(format!("glm_image in_channels {} vs {}", c, self.cfg.in_channels)));
        }
        let p = self.cfg.patch_size;
        if h % p != 0 || w % p != 0 {
            return Err(msg(format!("glm_image spatial {h}x{w} not divisible by patch {p}")));
        }
