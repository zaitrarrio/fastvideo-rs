...
pub fn write_slim(
    ...
    for (i, key) in keys.iter().enumerate() {
        ...
        shards.last_mut().expect("one shard").push(TensorSpec::new(
            key.clone(),
            view.dtype.clone(),
            view.shape.to_vec(),
        ));
    ...
    for (n, specs) in shards.iter().enumerate() {
        ...
        let metadata = [
            ("format", "pt"),
            ("slim_for", "MiniMax-H3 text conditioning"),
            ("hidden_states_tap", tap_text.as_str()),
            ("order", "load order of fastvideo_cudarc::llm"),
        ];
        let mut writer =
            SafetensorsWriter::create(&path, specs, &metadata).map_err(|e| msg(e.to_string()))?;
        for spec in specs {
            let view = store.view(&spec.name).map_err(|e| msg(e.to_string()))?;
            writer
                .write(&spec.name, view.bytes)
                .map_err(|e| msg(e.to_string()))?;
        }
        writer.finish().map_err(|e| msg(e.to_string()))?;
        crate::wan::log::info(format_args!(
            "slim text encoder: shard {}/{} written",
            n + 1,
            shards.len()
        ));
        files.push(path);
    ...
    Ok(SlimReport {
        ...
        tensors: keys.len(),
        bytes: total,
        skipped_tensors: store.keys().count() - keys.len(),
        skipped_bytes: all_bytes - total,
    })
}
...
pub fn write_slim_text_root(root: &Path, out: &Path, shard_bytes: u64) -> Result<SlimReport> {
    ...
    let report = write_slim(
        &store,
        &DecoderConfig::qwen3_vl_32b_text(),
        tap,
        &out.join("text_encoder"),
        shard_bytes,
    )?;
...
mod tests {
    ...
    fn publish(dir: &Path, cfg: &DecoderConfig) {
        ...
        for (n, part) in [&all[..half], &all[half..]].into_iter().enumerate() {
            let specs: Vec<TensorSpec> = part
                .iter()
                .map(|(k, s)| TensorSpec::new(k.clone(), LazyDType::BF16, s.clone()))
                .collect();
            let mut w = SafetensorsWriter::create(
                &dir.join(format!("model-{:05}-of-00002.safetensors", n + 1)),
                &specs,
                &[],
            )
            .unwrap();
            for (key, shape) in part {
                ...
                let bytes: Vec<u8> = (0..shape.iter().product::<usize>())
                    .flat_map(|i| {
                        let u = (seed.wrapping_add(i as u32).wrapping_mul(2_654_435_761) >> 8)
                            as f32
                            / (1u32 << 24) as f32;
                        let v = if key.contains("norm") {
                            0.5 + u
                        } else {
                            u - 0.5
                        };
    ...
    fn the_slim_set_is_exactly_what_the_loader_requests_in_order() {
        for (cfg, tap) in [(tiny(), 2usize), (DecoderConfig::gemma3_12b_text(), 1)] {
            ...
            crate::llm::hidden_states(&map, &small, &[1, 2], &[0, 1], &[true, true], &[tap])
                .unwrap();
            assert_eq!(
                *asked.lock().unwrap(),
                tap_keys(&small, tap),
    ...
    fn production_slim_is_embed_plus_50_layers_and_50_3_gb() {
        let doc: serde_json::Value =
            serde_json::from_str(include_str!("manifests/text_encoder.json")).unwrap();
        ...
        let bytes: u64 = keys
            ...
            .map(|k| {
                let t = tensors
                    .get(k)
                    .unwrap_or_else(|| panic!("{k} is not in the published checkpoint"));
                assert_eq!(t[0], "BF16");
                2 * t[1]
                    .as_array()
                    .unwrap()
        ...
        assert_eq!(
            (keys.len(), bytes),
            (551, 50_315_658_240),
            "46.86 GiB of the published 66.7 GB"
        );
    }
    ...
    fn slim_holds_the_same_bytes_encodes_identically_and_shares_the_cache() {
        ...
        assert_eq!(
            (report.files.len(), report.tensors),
            (3, 1 + 2 * 11),
        ...
        assert_eq!(
            ...
            3 + 2 * 11,
            "norm, lm_head, vision, layers 2 and 3"
        );
        assert!(
            report.files.windows(2).all(|w| w[0] < w[1]),
            "shard names sort in load order"
        );
        ...
        for key in &want {
            ...
            assert!(
                a.bytes == b.bytes && a.shape == b.shape && a.dtype == b.dtype,
                "{key}"
            );
        }
        assert_eq!(
            report.bytes,
            want.iter()
                .map(|k| packed.view(k).unwrap().bytes.len() as u64)
                .sum::<u64>()
        );
        assert_eq!(
            encoder_identity(&source, &cfg, tap).unwrap(),
            encoder_identity(&packed, &cfg, tap).unwrap(),
            "one encoder, two layouts"
        );
        assert!(
            write_slim(&source, &cfg, 4, &root.join("bad"), 1).is_err(),
            "the last tap needs the final norm"
        );

        // Same hidden state from either layout...
        let ids = [1u32, 4, 2];
        let (full_map, slim_map) = (
            WeightMap::open(&published).unwrap(),
            WeightMap::open(&slim).unwrap(),
        );
        let a = encode_ids(&full_map, &cfg, &ids, tap)
            .unwrap()
            .host_cow()
            .unwrap()
            .into_owned();
        let b = encode_ids(&slim_map, &cfg, &ids, tap)
            .unwrap()
            .host_cow()
            .unwrap()
            .into_owned();
        ...
        let key = |store: &LazyStore| {
            cache_key(
                "p",
                &[5; 32],
                tap,
                &encoder_identity(store, &cfg, tap).unwrap(),
            )
        };
        let encoder = StreamedEncoder {
            map: &full_map,
            cfg: &cfg,
        };
        let (_, hit) = get_or_compute(&cache, &key(&source), &ids, cfg.hidden, || {
            Ok(encoder.hidden_state(&ids, tap)?.host_cow()?.into_owned())
        })
        .unwrap();
        assert!(!hit);
        let (entry, hit) = get_or_compute(&cache, &key(&packed), &ids, cfg.hidden, || {
            panic!("a hit must not run the encoder")
        })
        .unwrap();
        assert!(hit);
