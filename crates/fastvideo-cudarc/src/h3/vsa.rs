...
impl H3TilePlan {
    pub fn new(layout: &H3PackedLayout) -> Result<Self> {
        ...
        if !contiguous {
            return Err(msg(
                "vsa-h3: the packed layout must be [text | cond | audio | video]",
            ));
        }
        ...
        if video.seq != layout.video.len {
            return Err(msg(format!(
                "vsa-h3: grid {:?} holds {} tokens, the layout has {} video rows",
                layout.token_grid, video.seq, layout.video.len
            )));
        }
...
pub fn attention_host(
    ...
    if [q, k, v].iter().any(|b| b.len() != want) || gate.is_some_and(|g| g.len() != want) {
        return Err(msg(format!(
            "vsa-h3: q/k/v/gate must each hold {want} values"
        )));
    }
    let scale = 1.0 / (dim as f64).sqrt();
    let tile_rows = |t: usize| {
        plan.slot_src[t * TILE_ELEMS..t * TILE_ELEMS + plan.block_sizes[t] as usize]
            .iter()
            .map(|&s| s as usize)
    };
        ...
        .for_each(|(h, out)| {
            ...
            let pool = |x: &[f32]| -> Vec<f64> {
                ...
                for t in 0..n {
                    ...
                    p[t * dim..(t + 1) * dim]
                        .iter_mut()
                        .for_each(|s| *s /= f64::from(plan.block_sizes[t]));
                }
                p
            };
            ...
            let scores: Vec<f32> = (0..n * n)
                .map(|ij| {
                    ((0..dim)
                        .map(|d| qbar[(ij / n) * dim + d] * kbar[(ij % n) * dim + d])
                        .sum::<f64>()
                        * scale) as f32
                })
                .collect();
            ...
            for i in 0..n {
                ...
                let mx = scores[i * n..(i + 1) * n]
                    .iter()
                    .cloned()
                    .fold(f32::MIN, f32::max);
                let weights: Vec<f64> = scores[i * n..(i + 1) * n]
                    .iter()
                    .map(|s| f64::from(s - mx).exp())
                    .collect();
                let z: f64 = weights.iter().sum();
                let compressed: Vec<f64> = (0..dim)
                    .map(|d| (0..n).map(|j| weights[j] / z * vbar[j * dim + d]).sum())
                    .collect();
                ...
                for r in tile_rows(i) {
                    let logits: Vec<f64> = keys
                        ...
                        .map(|&kr| {
                            (0..dim)
                                .map(|d| {
                                    f64::from(q[base + r * dim + d])
                                        * f64::from(k[base + kr * dim + d])
                                })
                                .sum::<f64>()
                                * scale
                        })
...
impl H3Vsa {
    pub fn new(
        ...
        let device = if crate::wan::stats::device_expected() {
            ...
            let mut bias = CudaTensor::from_vec(
                (0..n)
                    .map(|j| {
                        if j < plan.prefix_tiles {
                            PREFIX_BIAS
                        } else {
                            0.0
                        }
            ...
            bias.pin_device()?;
            let prefix_branch_rows = (0..heads)
                .flat_map(|h| {
                    plan.prefix_row_tile
                        .iter()
                        .map(move |&t| h * n + t as usize)
                })
                .collect();
            Some(DevicePlan {
                plan: crate::wan::ops::vsa_plan_upload(
                    &plan.slot_src,
                    &plan.block_sizes,
                    TILE_ELEMS,
                )?,
    ...
    pub fn attend(
        ...
        if q.shape != shape
            || k.shape != shape
            || v.shape != shape
            || gate.as_ref().is_some_and(|g| g.shape != shape)
        {
            return Err(msg(format!(
                "vsa-h3: q {:?} k {:?} v {:?} for a plan of {shape:?}",
    ...
    fn attend_device(
        ...
        fn slice(t: &CudaTensor) -> Result<&cudarc::driver::CudaSlice<f32>> {
            t.device_slice()
                .ok_or_else(|| msg("vsa-h3: tensor has no device buffer"))
        }
        ...
        if sm >= 8 && dim == 128 && !forced_gather {
            ...
            while q_base < n {
                ...
                let (kg, vg) = ops::vsa_gather_kv_device(
                    kd, vd, &selected, &dp.plan, bh, g, seq, dim, topk, q_base,
                )?;
                ...
                device::matmul_2d_strided_batched_bf16(
                    &p,
                    &vg,
                    &mut sparse,
                    bh * g,
        ...
        phase("vsa_h3_4_prefix_dense", || {
            let mut dense = crate::wan::nn::scaled_dot_product_attention(
                &q.narrow(2, 0, prefix)?,
                &k,
                &v,
                Some(scale),
...
mod tests {
    ...
    fn seeded(n: usize, k: f32) -> Vec<f32> {
        (0..n)
            .map(|i| ((i as f32 * k).sin() * 1.7 + (i as f32 * 0.013).cos()) * 0.6)
            .collect()
    }
    ...
    fn prefix_tiles_are_segment_pure_and_video_tiles_are_cubes() {
        ...
        assert_eq!(
            p.slot_src[64 + 6],
            -1,
            "a text tile is never topped up with audio"
        );
        ...
        assert_eq!(
            (
                p.prefix_row_tile[63],
                p.prefix_row_tile[64],
                p.prefix_row_tile[70],
                p.prefix_row_tile[135]
            ),
            (0, 1, 2, 3)
        );
    ...
    fn the_spec_tile_counts_hold_for_5_and_15_seconds() {
        ...
        for (seconds, prefix, video, k_vid) in [(5, 11, 660, 132), (15, 23, 1782, 357)] {
            let l = H3PackedLayout::from_geometry(&H3Geometry::default_16x9(seconds).unwrap(), 256)
                .unwrap();
            let p = H3TilePlan::new(&l).unwrap();
            assert_eq!(
                (p.prefix_tiles, p.video_tiles, p.k_vid(0.8)),
                (prefix, video, k_vid),
                "{seconds} s"
            );
            ...
            assert!(
                prefix < p.num_tiles(),
                "{seconds} s: prefix skip needs a video tail"
            );
        }
    }
    ...
    fn the_mask_exempts_prefix_columns_and_keeps_prefix_queries_dense() {
        ...
        assert_eq!(
            row(0),
            vec![0, 1, 2, 3, 4, 5],
            "a prefix query tile sees everything"
        );
        assert_eq!(row(1), vec![0, 1, 2, 3, 4, 5]);
        assert_eq!(
            row(2),
            vec![0, 1, 3],
            "prefix columns forced; one video column by score"
        );
        ...
        assert!(
            block_mask(&scores, 2, 4).iter().all(|&m| m),
            "k_vid = all video tiles is dense"
        );
    }
    ...
    fn with_the_gate_at_sparsity_zero_the_difference_is_the_compression_term() {
        ...
        let dense = scaled_dot_product_attention(&q, &k, &v, None).unwrap();
        let (qh, kh, vh, gh) = (
            q.host_cow().unwrap().into_owned(),
            k.host_cow().unwrap().into_owned(),
            v.host_cow().unwrap().into_owned(),
            gate.host_cow().unwrap().into_owned(),
        );
        ...
        let n = plan.num_tiles();
        let rows = |t: usize| {
            plan.slot_src[t * 64..t * 64 + plan.block_sizes[t] as usize]
                .iter()
                .map(|&s| s as usize)
                .collect::<Vec<_>>()
        };
        ...
        for i in 0..n {
            ...
            let logits: Vec<f32> = (0..n)
                .map(|j| {
                    qb.iter().zip(pool(&kh, j)).map(|(a, b)| a * b).sum::<f32>()
                        / (dim as f32).sqrt()
                })
                .collect();
    ...
    fn sparse_video_rows_attend_only_their_selected_tiles() {
        ...
        let (q, k, v) = (
            seeded(heads * seq * dim, 0.37),
            seeded(heads * seq * dim, 0.71),
            seeded(heads * seq * dim, 0.19),
        );
        ...
        for h in 0..heads {
            let base = h * seq * dim;
            assert!(
                sparse[base..base + prefix]
                    .iter()
                    .zip(&dense[base..base + prefix])
                    .all(|(a, b)| (a - b).abs() < 1e-6),
                "prefix rows stay dense"
            );
            assert!(
                sparse[base + prefix..base + seq * dim]
                    .iter()
                    .zip(&dense[base + prefix..base + seq * dim])
                    .any(|(a, b)| (a - b).abs() > 1e-4),
                "video rows are sparsified"
            );
        }
        ...
        assert_eq!(
            &*via_struct.host_cow().unwrap(),
            &sparse[..],
            "H3Vsa::attend is the host reference on CPU runs"
        );
    }
}
