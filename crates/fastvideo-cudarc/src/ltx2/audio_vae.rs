...
impl CausalConv2d {
    fn load(map: &WeightMap, prefix: &str, cin: usize, cout: usize, kernel: usize) -> Result<Self> {
        let mut weight = cuda_tensor_shaped(map, &format!("{prefix}.conv.weight"), &[cout, cin, kernel, kernel])?;
    ...
    fn forward(&self, x: &CudaTensor) -> Result<CudaTensor> {
        ...
        let x = x.pad(3, reach / 2, reach - reach / 2, PadMode::Zeros)?.pad(2, reach, 0, PadMode::Zeros)?;
...
impl Resnet {
    fn load(map: &WeightMap, prefix: &str, cin: usize, cout: usize) -> Result<Self> {
        Ok(Self {
            ...
            shortcut: if cin == cout { None } else { Some(CausalConv2d::load(map, &format!("{prefix}.nin_shortcut"), cin, cout, 1)?) },
            ones_in: ones(cin)?,
            ones_out: ones(cout)?,
        })
    }
    ...
    fn forward(&self, x: &CudaTensor, eps: f32) -> Result<CudaTensor> {
        let h = self.conv1.forward(&x.rms_norm_channels_act(&self.ones_in, eps, true)?)?;
        let h = self.conv2.forward(&h.rms_norm_channels_act(&self.ones_out, eps, true)?)?;
        match &self.shortcut {
...
impl AudioDecoder {
    ...
    pub fn load(map: &WeightMap, cfg: &Ltx2AudioVaeConfig) -> Result<Self> {
        if !cfg.causal_time_axis || cfg.mid_block_add_attention {
            return Err(msg("audio vae: only the time-causal, attention-free decoder of LTX-2.0 is supported"));
        }
        ...
        Ok(Self {
            ...
            mid: [Resnet::load(map, "decoder.mid.block_1", top, top)?, Resnet::load(map, "decoder.mid.block_2", top, top)?],
            levels,
            ones_out: ones(cin)?,
    ...
    pub fn decode_packed(&self, packed: &CudaTensor) -> Result<CudaTensor> {
        ...
        let [b, l, f] = packed.shape[..] else {
            return Err(msg(format!("audio latent must be packed [B, L, {}], got {:?}", c * m, packed.shape)));
        };
        if f != c * m || l == 0 {
            return Err(msg(format!("audio latent {:?}: expected {} features per frame", packed.shape, c * m)));
        }
    ...
    pub fn decode(&self, z: &CudaTensor) -> Result<CudaTensor> {
        let [_, c, frames, _] = z.shape[..] else {
            return Err(msg(format!("audio vae expects [B, C, L, M], got {:?}", z.shape)));
        };
        if c != self.cfg.latent_channels || frames == 0 {
            return Err(msg(format!("audio vae expects {} latent channels, got {:?}", self.cfg.latent_channels, z.shape)));
        }
        ...
        let x = self.conv_out.forward(&x.rms_norm_channels_act(&self.ones_out, eps, true)?)?;
        ...
        x.narrow(2, 0, t)?.narrow(3, 0, m)?.pad(2, 0, want_t - t, PadMode::Zeros)?.pad(3, 0, want_m - m, PadMode::Zeros)
    }
}
...
pub(crate) mod tests {
    ...
    fn causal_conv(x: &Map3, w: &[f32], b: &[f32], k: usize) -> Map3 {
            ...
            for y in 0..x.h {
                for xx in 0..x.w {
                    ...
                    for i in 0..x.c {
                        for ky in 0..k {
                            for kx in 0..k {
                                acc += w[((o * x.c + i) * k + ky) * k + kx] * x.at(i, y as isize + ky as isize - top, xx as isize + kx as isize - side);
                            }
                        }
                    }
        ...
        Map3 { c: cout, h: x.h, w: x.w, v }
    }

    fn pixel_norm_silu(x: &Map3, eps: f32) -> Map3 {
        let mut out = x.clone();
        for p in 0..x.h * x.w {
            let ms = (0..x.c).map(|c| x.v[c * x.h * x.w + p].powi(2)).sum::<f32>() / x.c as f32;
            for c in 0..x.c {
    ...
    fn conv(map: &WeightMap, prefix: &str, x: &Map3, cout: usize, k: usize) -> Map3 {
        causal_conv(x, &get(map, &format!("{prefix}.conv.weight"), &[cout, x.c, k, k]), &get(map, &format!("{prefix}.conv.bias"), &[cout]), k)
    }

    fn resnet(map: &WeightMap, prefix: &str, x: &Map3, cout: usize) -> Map3 {
        let h = conv(map, &format!("{prefix}.conv1"), &pixel_norm_silu(x, 1e-6), cout, 3);
        let h = conv(map, &format!("{prefix}.conv2"), &pixel_norm_silu(&h, 1e-6), cout, 3);
        let skip = if x.c == cout { x.clone() } else { conv(map, &format!("{prefix}.nin_shortcut"), x, cout, 1) };
        Map3 { v: skip.v.iter().zip(&h.v).map(|(a, b)| a + b).collect(), ..h }
    }

    fn tiny() -> Ltx2AudioVaeConfig {
        Ltx2AudioVaeConfig { base_channels: 2, ch_mult: [1, 2, 4], num_res_blocks: 1, latent_channels: 2, mel_bins: 8, ..Ltx2AudioVaeConfig::ltx2_19b() }
    }
    ...
    fn decoder_matches_a_loop_reference_and_grows_time_causally() {
        ...
        let got = dec.decode_packed(&CudaTensor::from_vec(packed.clone(), vec![1, l, 2 * m]).unwrap()).unwrap();
        assert_eq!(got.shape, vec![1, 2, 4 * l - 3, 8]);

        // De-normalise per packed feature, then unpack feature = channel·M + bin.
        let (mean, std) = (get(&map, "latents_mean", &[2 * m]), get(&map, "latents_std", &[2 * m]));
        let mut z = Map3 { c: 2, h: l, w: m, v: vec![0.0; 2 * l * m] };
        for t in 0..l {
            for f in 0..2 * m {
        ...
        for (level, cout) in [(2usize, 8usize), (1, 4), (0, 2)] {
            ...
            if level != 0 {
                let mut up = Map3 { c: x.c, h: 2 * x.h, w: 2 * x.w, v: vec![0.0; x.c * 4 * x.h * x.w] };
                for c in 0..x.c {
                    for y in 0..up.h {
                        for xx in 0..up.w {
                            up.v[(c * up.h + y) * up.w + xx] = x.at(c, (y / 2) as isize, (xx / 2) as isize);
                        }
                    }
                }
        ...
        for (i, (a, b)) in got.iter().zip(&want.v).enumerate() {
            assert!((a - b).abs() < 1e-4 * (1.0 + b.abs()), "mel[{i}]: {a} vs {b}");
        }
    }
    ...
    fn later_latent_frames_do_not_change_earlier_mel_frames() {
        ...
        let run = |v: &[f32]| dec.decode_packed(&CudaTensor::from_vec(v.to_vec(), vec![1, 4, 4]).unwrap()).unwrap().host_cow().unwrap().into_owned();
        let (a, b) = (run(&base), run(&changed));
        let (frames, bins) = (13usize, 8usize);
        for ch in 0..2 {
            for t in 0..frames {
                let same = (0..bins).all(|m| (a[(ch * frames + t) * bins + m] - b[(ch * frames + t) * bins + m]).abs() < 1e-6);
                assert_eq!(same, t < 9, "mel frame {t} (latent frame 3 starts at mel frame 9)");
            }
        }
    }
