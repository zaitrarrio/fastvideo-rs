...
impl Ctx<'_> {
    ...
    fn cmp(&mut self, name: &str, got: &[f32], want: &[f32], limit: f64) -> StageResult<()> {
        let d = diff(got, want);
        self.report
            .check(name, d.within(limit), d.to_json(), json!({"rel_l2": limit}))
    }
}
                            ...
                            for dt in 0..kt {
                                for dy in 0..kh {
                                    for dx in 0..kw {
                                        ...
                                        let xi = (((b * ic + c) * it + zt - pt) * ih + zy - ph)
                                            * iw
                                            + zx
                                            - pw;
...
pub fn run(report: &mut Report, lim: Limits, seed: u64) -> StageResult<()> {
    ...
    group(&mut c, "elementwise", |c| {
        for n in [1usize, 1000, 65_537] {
            ...
            c.cmp(
                &format!("mul_scalar_n{n}"),
                &down(&ops::mul_scalar_device(&da, -1.75)?)?,
                &a.iter().map(|v| v * -1.75).collect::<Vec<_>>(),
                op,
            )?;
            c.cmp(
                &format!("add_scalar_n{n}"),
                &down(&ops::add_scalar_device(&da, 0.5)?)?,
                &a.iter().map(|v| v + 0.5).collect::<Vec<_>>(),
                op,
            )?;
            let want: Vec<f32> = a.iter().map(|&v| v / (1.0 + (-v).exp())).collect();
            c.cmp(
                &format!("silu_n{n}"),
                &down(&ops::unary_device(&da, ops::ElemUnary::Silu)?)?,
                &want,
                op,
            )?;
            let half = (n / 2).max(1);
            ...
            c.cmp(
                &format!("swiglu_h{half}"),
                &down(&ops::swiglu_value_first_device(&up(&packed)?, half)?)?,
                &want,
                op,
            )?;
            let want: Vec<f32> = a.iter().map(|&v| ref_gelu_tanh(v)).collect();
            c.cmp(
                &format!("gelu_tanh_n{n}"),
                &down(&ops::unary_device(&da, ops::ElemUnary::GeluTanh)?)?,
                &want,
                op,
            )?;
            c.cmp(
                &format!("clamp_n{n}"),
                &down(&ops::clamp_device(&da, -0.3, 0.7)?)?,
                &a.iter().map(|v| v.clamp(-0.3, 0.7)).collect::<Vec<_>>(),
                op,
            )?;
            c.cmp(
                &format!("fill_n{n}"),
                &down(&ops::fill_device(n, 0.25)?)?,
                &vec![0.25; n],
                0.0,
            )?;
            ...
            c.cmp(
                &format!("lincomb5_n{n}"),
                &down(&ops::lincomb_device(&terms)?)?,
                &want,
                op,
            )?;
        }
        // Broadcast binary: trailing repeat, channel broadcast, leading batch.
        for (big_shape, inner, period) in [
            (vec![2usize, 6, 7], 1usize, 42usize),
            (vec![2, 5, 3, 4], 12, 5),
            (vec![3, 4, 5], 1, 5),
        ] {
            ...
            for bop in [
                ...
                let got = down(&ops::bcast_binary_device(
                    &dbig, &dsmall, inner, period, bop,
                )?)?;
                let want: Vec<f32> = (0..nb)
                    .map(|i| bop.apply(big[i], small[(i / inner) % period]))
                    .collect();
    ...
    group(&mut c, "row_reductions", |c| {
        ...
        for (rows, width) in [
            ...
            let dx = up(&x)?;
            c.cmp(
                &format!("softmax_{rows}x{width}"),
                &down(&ops::softmax_last_device(&dx, width)?)?,
                &ref_softmax(&x, width),
                op,
            )?;
            ...
            let got: Vec<f32> = dev()?
                ...
                .collect();
            c.cmp(
                &format!("softmax_bf16_{rows}x{width}"),
                &got,
                &ref_softmax(&x, width),
                8e-3,
            )?;
            ...
            let (dw, db) = (up(&w)?, up(&b)?);
            c.cmp(
                &format!("rms_norm_{rows}x{width}"),
                &down(&ops::rms_norm_last_device(&dx, &dw, 1e-6)?)?,
                &ref_rms(&x, &w, 1e-6),
                op,
            )?;
            let got = down(&ops::layer_norm_last_device(
                &dx,
                Some((&dw, &db)),
                width,
                1e-6,
            ...
            c.cmp(
                &format!("layer_norm_affine_{rows}x{width}"),
                &got,
                &ref_layer_norm(&x, width, Some((&w, &b)), 1e-6),
                op,
            )?;
            let got = down(&ops::layer_norm_last_device(&dx, None, width, 1e-6)?)?;
            c.cmp(
                &format!("layer_norm_plain_{rows}x{width}"),
                &got,
                &ref_layer_norm(&x, width, None, 1e-6),
                op,
            )?;
        }
        Ok(())
    ...
    group(&mut c, "dit_fused", |c| {
        for (b, s, d) in [(1usize, 17usize, 64usize), (2, 300, 1536)] {
            ...
            c.cmp(
                &format!("ln_adaln_e_{b}x{s}x{d}"),
                &host_of(&xt.ln_adaln_e(&et, 4, 3, 1e-6)?)?,
                &want,
                op,
            )?;
            c.cmp(
                &format!("residual_gate_add_e_{b}x{s}x{d}"),
                &host_of(&xt.residual_gate_add_e(&ut, &et, 5)?)?,
                &want_gate,
                op,
            )?;
            ...
            c.cmp(
                &format!("add_bias_last_{b}x{s}x{d}"),
                &down(&out)?,
                &x.iter()
                    .enumerate()
                    .map(|(i, v)| v + bias[i % d])
            ...
            let want: Vec<f32> = x
                .iter()
                .enumerate()
                .map(|(i, v)| ref_gelu_tanh(v + bias[i % d]))
                .collect();
        ...
        for (b, s, heads, hd) in [(1usize, 9usize, 2usize, 8usize), (2, 128, 12, 128)] {
            ...
            for (col, rope) in [(0usize, true), (1, true), (1, false)] {
                let got = xt.qk_norm_rope_bhsd(
                    col * width,
                    heads,
                    &wt,
                    rope.then_some(Rope { cos: &ct, sin: &st }),
                    1e-6,
                )?;
                let slice: Vec<f32> = (0..b * s)
                    .flat_map(|r| {
                        x[r * 3 * width + col * width..r * 3 * width + (col + 1) * width].to_vec()
                    })
                    .collect();
                ...
                c.cmp(
                    &format!("qk_norm_rope_bhsd_col{col}_rope{rope}_{b}x{s}x{heads}x{hd}"),
                    &host_of(&got)?,
                    &want,
                    op,
                )?;
            ...
            let want: Vec<f32> = (0..b * s)
                .flat_map(|r| x[r * 3 * width + 2 * width..r * 3 * width + 3 * width].to_vec())
                .collect();
            c.cmp(
                &format!("split_merge_heads_{b}x{s}x{heads}x{hd}"),
                &merged,
                &want,
                0.0,
            )?;
    ...
    group(&mut c, "data_movement", |c| {
        for (shape, perm) in [
            ...
            c.cmp(
                &format!("gather_nd_{shape:?}_{perm:?}"),
                &got,
                &ref_permute(&x, &shape, &perm),
                0.0,
            )?;
        }
        ...
        for dim in 0..shape.len() {
            ...
            c.cmp(
                &format!("cat_dim{dim}"),
                &host_of(&CudaTensor::cat(&[&first, &a], dim)?)?,
                &x,
                0.0,
            )?;
        ...
        }
        {
            ...
            c.cmp(
                "upsample_nearest_2x",
                &got,
                &host::upsample_nearest(&x, nc, h, w, 2, 2),
                0.0,
            )?;
        ...
        }
        {
            ...
            let got = down(&ops::rms_norm_channels_device(
                &up(&x)?,
                &up(&g)?,
                n,
                ch,
            ...
            for ni in 0..n {
                for s in 0..spatial {
                    let acc: f64 = (0..ch)
                        .map(|ci| f64::from(x[(ni * ch + ci) * spatial + s]).powi(2))
                        .sum();
            ...
            let fused = down(&ops::rms_norm_channels_device(
                &up(&x)?,
                &up(&g)?,
                n,
                ch,
        ...
        }
        {
            ...
            let want: Vec<f32> = idx
                .iter()
                .flat_map(|&i| table[i * d..(i + 1) * d].to_vec())
                .collect();
    ...
    group(&mut c, "gemm", |c| {
        for (m, kk, n) in [
            ...
            c.cmp(
                &format!("gemm_2d_{m}x{kk}x{n}"),
                &down(&out)?,
                &ref_matmul(&a, &b, m, kk, n),
                gemm,
            )?;
            ...
            c.cmp(
                &format!("gemm_linear_{m}x{kk}x{n}"),
                &down(&out)?,
                &ref_matmul(&a, &transpose2(&w, n, kk), m, kk, n),
                gemm,
            )?;
        }
        {
        ...
        }
        {
            ...
            let got = down(&ops::cast_bf16_f32_bias_act_device(
                &x16,
                Some(&up(&bias)?),
                true,
            )?)?;
            let want: Vec<f32> = want
                .iter()
                .enumerate()
                .map(|(i, v)| ref_gelu_tanh(v + bias[i % 7]))
                .collect();
        ...
        }
        {
            ...
            c.cmp(
                "leaky_relu",
                &got,
                &host::map1(&x, |v| host::leaky_relu(v, 0.1)),
                0.0,
            )?;
            ...
            let got = down(&ops::snake_beta_device(
                &up(&x)?,
                &up(&alpha)?,
                &up(&inv_beta)?,
                ch,
            ...
            c.cmp(
                "snake_beta",
                &got,
                &host::snake_beta(&x, &alpha, &inv_beta, ch, l),
                op,
            )?;
            ...
            for (b, h, s, d, r) in [
                ...
                c.cmp(
                    &format!("rope_half_d{d}_r{r}"),
                    &got,
                    &host::rope_half(&x, &cs, &sn, s, d, r),
                    op,
                )?;
            }
        ...
        }
        {
            ...
            for (rows, cols) in [(64usize, 5120usize), (257, 1000), (3, 25600)] {
                ...
                c.cmp(
                    &format!("fp8_rows_scales_{rows}x{cols}"),
                    &down(&scales)?,
                    &hs,
                    0.0,
                )?;
                let w16: Vec<f32> = dev()?
                    ...
                    .collect();
                c.cmp(
                    &format!("fp8_rows_dequant_bf16_{rows}x{cols}"),
                    &w16,
                    &host::fp8_rows_dequant(&hq, &hs, cols),
                    0.0,
                )?;
            }
        }
            ...
            for bits in [8u8, 6, 4] {
                ...
                for (r, row) in w.chunks_mut(cols).enumerate() {
                    let g = if r == 2 {
                        ...
                        4f32.powi((r % 3) as i32 - 1)
                    };
                    row.iter_mut().for_each(|v| *v *= g);
                }
                let (hq, hs, hb) =
                    affine::quantize(&w, rows, cols, bits).map_err(|e| anyhow::anyhow!("{e}"))?;
                let (q, scales, biases) =
                    ops::affine_quantize_device(&up(&w)?, rows, cols, bits, affine::GROUP)?;
                let dq = dev()?.stream.memcpy_dtov(&q)?;
                let mism = dq.iter().zip(&hq).filter(|(a, b)| a != b).count();
                c.report.check(
                ...
                c.cmp(
                    &format!("affine_scales_int{bits}"),
                    &down(&scales)?,
                    &hs,
                    0.0,
                )?;
                c.cmp(
                    &format!("affine_biases_int{bits}"),
                    &down(&biases)?,
                    &hb,
                    0.0,
                )?;
                let got = down(&ops::affine_dequant_device(
                    &q,
                    &scales,
                    &biases,
                    cols,
                ...
                let yg_host = affine::gemm(&x, &hq, &hs, &hb, m, n, k, bits)
                    .map_err(|e| anyhow::anyhow!("{e}"))?;
        ...
        }
        {
            ...
            let dir =
                std::env::temp_dir().join(format!("fv-gpucheck-prefetch-{}", std::process::id()));
            // Prefetching needs bf16 linears, which exact mode turns off: the
            // fast-mode kernels stage is where this runs.
            let stored_kinds: &[bool] = if fastvideo_cudarc::wan::nn::bf16_linears_active() {
                &[true, false]
            } else {
                &[]
            };
        ...
        }
        {
            ...
            for (mode, name) in [
                ...
                c.cmp(
                    &format!("pad_axis_{name}"),
                    &got,
                    &host::pad_axis(&x, len, inner, 5, 3, mode),
                    0.0,
                )?;
            }
            for (n, ch, spatial, groups) in [
                (1usize, 128usize, 4 * 96 * 168usize, 32usize),
                (3, 32, 7, 32),
                (2, 512, 1024, 32),
            ] {
                // An offset mean makes the variance a small difference of large sums.
                let x: Vec<f32> = c
                    .rand(n * ch * spatial, 0.5)
                    .iter()
                    .map(|v| v + 3.0)
                    .collect();
                ...
                for silu in [false, true] {
                    let got = down(&ops::group_norm_device(
                        &up(&x)?,
                        &up(&w)?,
                        &up(&b)?,
                        n,
        ...
        }
        {
            ...
            for (name, ch, oc, k, pad, stride, dil, groups, l) in [
                ...
                let b = c.rand(oc, 0.1);
                let got = t(x.clone(), &[2, ch, l])?.conv1d(
                    &t(w.clone(), &[oc, ch / groups, k])?,
                    Some(&t(b.clone(), &[oc])?),
                    pad,
                    stride,
                    dil,
                ...
                let (mut want, lo) =
                    ops::host::conv1d(&x, (2, ch, l), &w, (oc, k), pad, stride, dil, groups);
            ...
            for (name, ch, og, k, pad, stride, groups, out_pad, l) in [
                ...
                let got = t(x.clone(), &[2, ch, l])?.conv_transpose1d(
                    &t(w.clone(), &[ch, og, k])?,
                    None,
                    pad,
                    stride,
                ...
                let (want, _) = ops::host::conv_transpose1d(
                    &x,
                    (2, ch, l),
                    &w,
                    (og, k),
        ...
        }
        {
            ...
            for i in 0..frames * plane {
                ...
                for ch in 0..3 {
                    want.push(
                        ((x[(f * 3 + ch) * plane + p] + 1.0) * 127.5).clamp(0.0, 255.0) as u8,
                    );
                }
            }
        ...
        for (rows, kk, n, gelu) in [
            ...
            let lin = Linear::from_tensors(
                CudaTensor::from_vec(w.clone(), vec![n, kk])?,
            ...
            c.cmp(
                &format!("linear_{math:?}_{rows}x{kk}x{n}_gelu{gelu}"),
                &host_of(&got)?,
                &want,
                gemm,
            )?;
        }
        {
            ...
            let want: Vec<f32> = (0..batch)
                .flat_map(|bi| ref_matmul(&w, &x[bi * ic * s..(bi + 1) * ic * s], oc, ic, s))
                .collect();
    ...
    group(&mut c, "vsa", |c| {
        ...
        for (grid, heads, dim, sparsity) in [
            ...
            let want =
                vsa::vsa_attention_host(&q, &k, &v, Some(&g), &plan, topk, b, heads, dim, scale)?;
            ...
            let plan_dev =
                ops::vsa_plan_upload(&plan.slot_src, &plan.block_sizes, vsa::TILE_ELEMS)?;
            ...
            for group in [nb, 2.min(nb)] {
                ...
                let tag = format!(
                    "vsa_{}x{}x{}_h{heads}_d{dim}_k{topk}_g{group}",
            ...
            let tag = format!(
                "vsa_fused_{}x{}x{}_h{heads}_d{dim}_k{topk}",
                grid.0, grid.1, grid.2
            );
            ...
            if dim == 128 && dev.sm_major >= 8 {
                ...
                std::env::remove_var("FASTVIDEO_VSA_KERNEL");
                let tag = format!(
                    "vsa_mma_{}x{}x{}_h{heads}_d{dim}_k{topk}",
                    grid.0, grid.1, grid.2
                );
                ...
                if dev.sm_major >= 9 {
                    ...
                    std::env::remove_var("FASTVIDEO_VSA_KERNEL");
                    let tag = format!(
                        "vsa_tma_{}x{}x{}_h{heads}_d{dim}_k{topk}",
                        grid.0, grid.1, grid.2
                    );
                    c.cmp(&tag, &dev.stream.memcpy_dtov(&got)?, &want, 2e-2)?;
                }
            ...
            for h in 0..bh {
                for t in 0..nb {
                    ...
                    for sl in t * vsa::TILE_ELEMS..t * vsa::TILE_ELEMS + cnt {
                        ...
                        for d in 0..dim {
                            want_mean[(h * nb + t) * dim + d] +=
                                q[(h * seq + tok) * dim + d] / cnt as f32;
                        }
                    }
                }
            }
            c.cmp(
                &format!("vsa_tile_mean_{}x{}x{}", grid.0, grid.1, grid.2),
                &down(&qc)?,
                &want_mean,
                op,
            )?;
    ...
    group(&mut c, "attention", |c| {
        ...
        for (b, h, sq, sk, d, budget) in [
            ...
            c.cmp(
                &format!("dense_sdpa{chunked}_{tag}"),
                &host_of(&dense)?,
                &want,
                gemm.max(op),
            )?;
            if d % 32 == 0 && d <= attn::FLASH_MAX_HEAD_DIM {
                let got = attn::device_flash_sdpa(&qt, &kt, &vt, Some(scale))?
                    .ok_or_else(|| anyhow::anyhow!("flash declined"))?;
                c.cmp(
                    &format!("flash_sdpa_{tag}"),
                    &host_of(&got)?,
                    &want,
                    op.max(1e-4),
                )?;
    ...
    group(&mut c, "conv", |c| {
        ...
        for (pad, stride) in [(0usize, 1usize), (1, 1), (1, 2)] {
            ...
            c.cmp(
                &format!("cudnn_conv2d_p{pad}_s{stride}"),
                &host_of(&got)?,
                &want,
                gemm.max(op),
            )?;
        ...
        for (dims, stride) in [
            ((1usize, 4usize, 5usize, 9usize, 11usize), [1usize, 1, 1]),
            ((2, 3, 4, 10, 12), [1, 2, 2]),
        ] {
            ...
            let (y, y_shape) =
                conv::conv3d_unfold(x_dev, &xt.shape, w_dev, &wt.shape, [1, 1], stride)?;
            let got = CudaTensor::from_device_slice(y, y_shape)?.add_bias(&t(bias, &[oc])?, 1)?;
            c.cmp(
                &format!("unfold_conv3d_{tag}"),
                &host_of(&got)?,
                &want,
                gemm.max(op),
            )?;
        ...
        let got = t(x.clone(), &[n, ic, tt, hh, ww])?.conv3d(
            &t(w.clone(), &[oc, ic, 1, 1, 1])?,
            None,
            [0, 0, 0],
            [1, 1, 1],
        )?;
    ...
    group(&mut c, "conv3d_bench", |c| {
        ...
        for (ch, t_in, hh, ww) in [
            (384usize, 3usize, 30usize, 52usize),
            (192, 6, 120, 208),
            (96, 6, 240, 416),
        ] {
            ...
            c.report.note(
                format!("conv3d_bench_{ch}x{t_in}x{hh}x{ww}"),
    ...
    group(&mut c, "no_host_fallback", |c| {
        ...
        c.report.check(
            "generic_broadcast_refuses_host_compute",
            refused,
            json!({"refused": refused}),
            json!({}),
        )?;
